...
<?php include("../Helpers/execution_time.php"); ?>
    ...
    <div class="landing">
                    ...
                    <table class="table table-bordered table-hover table-striped table-group-divider text-center">
                        ...
                        if ($result_count > 0) {
                            ...
                            while ($row = mysqli_fetch_array($cart_result)) {
                                ...
                                $total_quantity += $product_quantity; // Add to total quantity
                        ...
                        ?>
                                    ...
                                    <tr class="d-flex flex-column d-md-table-row ">
                                        ...
                                        <td>
                                            <?php echo $price_table * $product_quantity; ?>$
                                        </td>
                                        <td>
                                            <input type="submit" value="Update" class="btn btn-dark" name="update_cart">
                                            <input type="submit" value="Remove" class="btn btn-primary" name="remove_cart" onclick="return confirm('Are you sure you want to remove this item?');">
                                        </td>
                                    </tr>
                        <?php   }
                            }
     ...
     function remove_cart_item()
     {
         ...
         // Check if remove_cart is set
         if (isset($_POST['remove_cart'])) {
             // Get the product ID from the form
             foreach ($_POST as $key => $value) {
                 if (strpos($key, 'qty_') === 0) {
                     $product_id = str_replace('qty_', '', $key); // Extract product ID
     ...
     function update_cart_quantity() {
         ...
         // Check if update_cart is set
         if (isset($_POST['update_cart'])) {
             // Loop through all POST data to find quantity fields
             foreach ($_POST as $key => $value) {
                 if (strpos($key, 'qty_') === 0) {
                     $product_id = str_replace('qty_', '', $key); // Extract product ID
                     $quantity = intval($value); // Get the new quantity
